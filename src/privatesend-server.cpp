...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if(fLiteMode) return; // ignore all Confederatestatesofamerica related functionality
...
/*
    ...
    allow endless transaction that would bloat Confederatestatesofamerica and make it unusable. To
...
bool CPrivateSendServer::CreateNewSession(const CDarksendAccept& dsa, PoolMessage& nMessageIDRet, CConnman& connman)
{
    ...
    nSessionID = GetRandInt(177699)+1;
...
{
    if(fLiteMode) return; // disable all Confederatestatesofamerica specific functionality
    ...
    RenameThread("confederatestatesofamerica-ps-server");
