...
//Confederatestatesofamerica only features
...
const char * const BITCOIN_CONF_FILENAME = "confederatestatesofamerica.conf";
const char * const BITCOIN_PID_FILENAME = "confederatestatesofamericad.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "confederatestatesofamerica" is a composite category enabling all Confederatestatesofamerica-related debug output
                if(ptrCategory->count(std::string("confederatestatesofamerica"))) {
...
#else
    const char* pszModule = "confederatestatesofamerica";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ConfederatestatesofamericaCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ConfederatestatesofamericaCore
    // Mac: ~/Library/Application Support/ConfederatestatesofamericaCore
    // Unix: ~/.confederatestatesofamericacore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ConfederatestatesofamericaCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ConfederatestatesofamericaCore";
#else
    // Unix
    return pathRet / ".confederatestatesofamericacore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty confederatestatesofamerica.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override confederatestatesofamerica.conf
